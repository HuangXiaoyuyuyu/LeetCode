package LeetCode_0657_机器人能否返回原点;

/**
 * @Author OliverYu
 * @Date 2019/8/2 22:28
 * @Email 253757635@qq.com
 * @Description 本题比较明显，如果moves字符串为奇数，就回不到原点，
 *              否则判断U和D以及L和R的数量关系。
 *              判断的时候建议使用switch而不是if-else if-else if-else。
 *
...
public class Solution {
    public boolean judgeCircle(String moves) {
        ...
        for (char c : moves.toCharArray()) {
            switch (c) {
                case 'U': countUD += 1;break;
                case 'D': countUD -= 1;break;
                case 'L': countLR += 1;break;
                case 'R': countLR -= 1;break;
            }
        }
